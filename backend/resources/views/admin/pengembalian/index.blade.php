...
@section('header-title', 'Manajemen Data Pengembalian Alat')
    ...
    <div class="p-5 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
            ...
            <form action="{{ route('admin.pengembalian.index') }}" method="GET" class="flex w-full md:w-80">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama peminjam / alat / status..."
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-gray-700 text-sm">
                ...
                <tr class="hover:bg-gray-50 transition align-top border-b">
                    ...
                    <td class="py-3 px-4 text-xs">
                        @if($peminjaman->pengembalian)
                            <span class="font-semibold text-emerald-700">{{ $peminjaman->pengembalian->tgl_kembali }}</span>
                        @else
                    ...
                    <td class="py-3 px-4 text-xs text-right font-medium">
                        @if($peminjaman->pengembalian)
                            <span class="text-gray-900">{{ number_format($peminjaman->pengembalian->denda ?? 0, 0, ',', '.') }}</span>
                        @else
                            <span class="text-gray-500 italic">-</span>
                        @endif
                    </td>
                    <td class="py-3 px-4 text-xs">
                        @if($peminjaman->pengembalian && $peminjaman->pengembalian->petugas)
                            <span class="font-medium">{{ $peminjaman->pengembalian->petugas->name }}</span>
                        @else
                            <span class="text-gray-500 italic">-</span>
                        @endif
                    </td>
